...
import './styles.css'
// FOUNDRY: the DESIGN.md skin, imported after styles.css so it wins on ties.
import './foundry/theme.css'
import { anim } from './anim'
// FOUNDRY: configureApp() moved into applyBrand() — see the note below.
...
// FOUNDRY: resolve the brand FIRST. It sets the chrome palette
// (<html data-brand>), the theme a new deck is born with, AND the kernel app
// identity — upstream's configureApp() call lives in applyBrand() (foundry/boot.ts)
// instead of here, because the identity (window-title suffix, save-picker label)
// changes with the brand and must be re-declared when it does. So this line is
...
// FOUNDRY: AFTER the pristine capture, deliberately. The @font-face block is
// ~190KB of base64 and already lives in the bundle — capturing it would copy it
...
function bootWith(doc: BentoDoc) {
  // FOUNDRY: a deck already wearing a brand's theme brings its chrome with it —
  // a Gitwork deck opens in Gitwork for whoever was sent it. `?brand=` still wins.
  adoptDeckBrand(doc)
