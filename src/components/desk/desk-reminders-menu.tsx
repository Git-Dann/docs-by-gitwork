...
export function DeskRemindersMenu() {
  ...
  // The clipboard is the owner's own to-do list — typed here. Slack /desk items
  // are excluded (they surface in "Needs you today").
  const items = (reminders.data?.reminders ?? []).filter((r) => r.source !== "SLACK");
...
function RemindersPanel() {
  ...
  // Owner's own to-dos only — Slack /desk items live in "Needs you today".
  const items = (reminders.data?.reminders ?? []).filter((r) => r.source !== "SLACK");
