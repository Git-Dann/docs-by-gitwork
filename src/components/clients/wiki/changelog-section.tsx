...
interface Props {
  ...
  /**
   * Which platforms are enabled for this wiki. Determines which tabs are shown.
   * Defaults to ["IOS","ANDROID","WEB"] (FIRESTICK is opt-in).
   */
  platforms?: string[];
...
export function ChangelogSection({
  ...
  // Build tab list: always start with "ALL", then the enabled platforms in display order.
  const ALL_ORDERED: FilterPlatform[] = ["IOS", "ANDROID", "FIRESTICK", "WEB"];
  const activePlatforms = enabledPlatforms ?? ["IOS", "ANDROID", "WEB"];
  const platforms: FilterPlatform[] = [
    "ALL",
    ...ALL_ORDERED.filter((p) => activePlatforms.includes(p)),
  ];
