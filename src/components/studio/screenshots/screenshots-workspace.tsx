...
// ── Background editor ──
function BackgroundEditor({ theme, onChange }: { theme: BackgroundTheme; onChange: (t: BackgroundTheme) => void }) {
  const fill = theme.fill;
  const setFill = (f: Fill) => onChange({ fill: f });
  const setStop = (i: number, color: string) => {
    if (fill.kind === "solid") return;
    const stops = fill.stops.map((s, j) => (j === i ? { ...s, color } : s));
    setFill({ ...fill, stops });
  };
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {BACKGROUND_PRESETS.map((p) => (
          <button
            ...
            className="h-8 w-8 rounded-[8px] border border-[var(--border-1)] transition hover:border-[var(--text-4)]"
            style={resolveSwatch(p.theme.fill)}
          />
        ))}
      </div>
      <Segmented
        ...
        onChange={(k) => {
          ...
          else if (k === "linear") setFill({ kind: "linear", angle: 160, stops: gradientStops(fill) });
...
function resolveSwatch(fill: Fill): React.CSSProperties {
