...
import { runNativeMobileChecks, runChromeExtensionChecks, runDesktopChecks, runCliChecks, runWebSourceChecks } from "@/server/pulse-checks/native-repo";
...
async function runRestOnlyFamilies(
  ...
  const [secretResult, nativeResult, extensionResult, desktopResult, cliResult, webResult] = await Promise.allSettled([
  ...
  for (const result of [nativeResult, extensionResult, desktopResult, cliResult, webResult]) {
