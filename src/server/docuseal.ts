...
export async function createDocuSealSubmission(
  ...
  // 2) Or create a new template on-the-fly via POST /templates/html (or POST /templates)
  let targetTemplateId: string | number | undefined =
    input.templateId ?? (defaultTemplateId || undefined);

  if (!targetTemplateId) {
    const htmlContent = input.html || `<h1>${input.title}</h1><p>Document generated by Gitwork</p>`;

    // Primary: POST /templates/html (DocuSeal standard endpoint for HTML templates)
    ...
    // Fallback: If /templates/html returned 404, try POST /templates with documents array
