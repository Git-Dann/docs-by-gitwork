...
export async function runMobileExtended(ctx: ExtendedCheckContext): Promise<PulseScanCheckInput[]> {
  ...
  if (platformIs(ctx.platform, "API_BACKEND", "CLI_TOOL")) {
    return skip("Mobile & Accessibility", [
  ...
  checks.push({ category: "Mobile & Accessibility", checkKey: "web_push_notifications", label: "Web Push Notifications support", status: hasWebPush ? "PASS" : "WARN", detail: hasWebPush ? "Web Push Notifications signals detected." : "No Web Push support detected — Web Push is a high-engagement re-activation channel for web apps, now supported on all major platforms including iOS 16.4+." });
  ...
  checks.push({ category: "Mobile & Accessibility", checkKey: "push_permission_polite", label: "Polite push permission prompt", status: hasImmediatePush ? "WARN" : "PASS", detail: hasImmediatePush ? "Immediate push permission request detected — browsers may suppress or block immediate permission prompts. Trigger after user interaction." : "No immediate push permission trigger detected." });
  ...
  checks.push({ category: "Mobile & Accessibility", checkKey: "apple_app_clip_support", label: "App Clips support (iOS)", status: isIosApp ? (hasAppClips ? "PASS" : "WARN") : "PASS", detail: isIosApp ? (hasAppClips ? "App Clips signals detected." : "iOS app signals detected but no App Clips — App Clips enable instant app experiences without full installation, great for onboarding.") : "Not applicable." });
