...
export async function runSecurityExtended(ctx: ExtendedCheckContext): Promise<PulseScanCheckInput[]> {
  ...
  if (platformIs(ctx.platform, "IOS_APP", "ANDROID_APP")) {
    return skip("Security", CHECKS, "Not applicable — native mobile apps are not web servers.");
  }
  ...
  checks.push({ category: "Security", checkKey: "cross_origin_opener_policy", label: "Cross-Origin-Opener-Policy (COOP)", status: hasCoop ? "PASS" : "WARN", detail: hasCoop ? `COOP header present: ${h["cross-origin-opener-policy"]}` : "No COOP header — prevents cross-origin window attacks (Spectre). Set to same-origin or same-origin-allow-popups." });

  // CORP
  const hasCorp = !!h["cross-origin-resource-policy"];
  checks.push({ category: "Security", checkKey: "cross_origin_resource_policy", label: "Cross-Origin-Resource-Policy (CORP)", status: hasCorp ? "PASS" : "WARN", detail: hasCorp ? `CORP header: ${h["cross-origin-resource-policy"]}` : "No CORP header — resources can be embedded by any origin. Set to same-site or same-origin to prevent cross-origin information leakage." });

  // COEP
  const hasCoep = !!h["cross-origin-embedder-policy"];
  checks.push({ category: "Security", checkKey: "cross_origin_embedder_policy", label: "Cross-Origin-Embedder-Policy (COEP)", status: hasCoep ? "PASS" : "WARN", detail: hasCoep ? `COEP header: ${h["cross-origin-embedder-policy"]}` : "No COEP header — required alongside COOP to enable cross-origin isolation and SharedArrayBuffer." });

  // CSP report directive
  const csp = h["content-security-policy"] ?? "";
  const hasReportUri = csp.includes("report-uri") || csp.includes("report-to");
  checks.push({ category: "Security", checkKey: "csp_report_directive", label: "CSP report-uri / report-to configured", status: hasReportUri ? "PASS" : "WARN", detail: hasReportUri ? "CSP report endpoint configured — policy violations are captured." : "No CSP report endpoint — without report-uri/report-to you have no visibility into CSP violations or injection attempts." });

  // Rate limiting headers
  const hasRateLimit = !!h["x-ratelimit-limit"] || !!h["x-ratelimit-remaining"] || !!h["retry-after"] || !!h["ratelimit-limit"];
  checks.push({ category: "Security", checkKey: "rate_limiting_headers", label: "Rate-limiting headers present", status: hasRateLimit ? "PASS" : "WARN", detail: hasRateLimit ? "Rate limiting headers detected — API endpoint protection in place." : "No rate-limiting headers — consider adding X-RateLimit-* headers to API responses to signal throttling behaviour." });

  // CAA DNS record
  const caaRecords = await checkDnsRecord(hostname, "CAA");
  const hasCaa = caaRecords.length > 0;
  checks.push({ category: "Security", checkKey: "caa_dns_record", label: "CAA DNS record (cert authority restriction)", status: hasCaa ? "PASS" : "WARN", detail: hasCaa ? `CAA record found — only authorised CAs can issue certificates for this domain.` : "No CAA record — any certificate authority can issue SSL certificates for your domain. Add a CAA record to restrict issuance to your CA." });

  // DNSSEC
  const dsRecords = await checkDnsRecord(hostname, "DS");
  const hasDnssec = dsRecords.length > 0;
  checks.push({ category: "Security", checkKey: "dnssec_enabled", label: "DNSSEC enabled on domain", status: hasDnssec ? "PASS" : "WARN", detail: hasDnssec ? "DNSSEC DS record found — DNS responses are cryptographically signed." : "No DNSSEC detected — DNS responses are unauthenticated and vulnerable to cache poisoning attacks." });
  ...
  checks.push({ category: "Security", checkKey: "no_api_keys_in_html", label: "No API key patterns in HTML source", status: hasExposedKeys ? "FAIL" : "PASS", detail: hasExposedKeys ? "CRITICAL: Potential API key or secret detected in page HTML source. Rotate affected credentials immediately." : "No obvious API key patterns detected in HTML source." });

  // CSRF tokens
  const hasCsrf = /name=["']_csrf["']|name=["']csrf_token["']|name=["']authenticity_token["']|csrf-token/i.test(pageResult.html);
  checks.push({ category: "Security", checkKey: "csrf_protection_signals", label: "CSRF token protection detected", status: hasCsrf ? "PASS" : "WARN", detail: hasCsrf ? "CSRF token detected in page HTML — form submissions are protected against cross-site request forgery." : "No CSRF token detected — ensure state-changing requests use CSRF protection (synchroniser tokens or SameSite cookies)." });
  ...
  checks.push({ category: "Security", checkKey: "dependency_audit_clean", label: "No obvious vulnerable library versions", status: hasOldLib ? "WARN" : "PASS", detail: hasOldLib ? "Outdated library version detected — check npm audit / Dependabot for known CVEs." : "No obviously vulnerable library versions detected in page source." });
  ...
  checks.push({ category: "Security", checkKey: "subdomain_takeover_risk", label: "No dangling CNAME / subdomain takeover risk", status: subTakeoverRisk ? "FAIL" : "PASS", detail: subTakeoverRisk ? "CNAME points to a cloud service that may be unclaimed — subdomain takeover risk. Verify the target resource still exists." : "No obvious dangling CNAME records detected." });

  // CSP nonce
  const hasCspNonce = csp.includes("nonce-") || /nonce=["'][^"']+["']/i.test(pageResult.html);
  const hasUnsafeInline = csp.includes("unsafe-inline");
  checks.push({ category: "Security", checkKey: "content_security_policy_nonce", label: "CSP uses nonces (not unsafe-inline)", status: hasCspNonce ? "PASS" : hasUnsafeInline ? "WARN" : "WARN", detail: hasCspNonce ? "CSP nonce detected — inline scripts are controlled and unsafe-inline is not needed." : csp ? "CSP present but uses unsafe-inline — migrate to nonce-based CSP for stronger XSS protection." : "No CSP detected — implement a nonce-based Content-Security-Policy." });
