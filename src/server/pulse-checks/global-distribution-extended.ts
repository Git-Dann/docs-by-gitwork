...
export async function runGlobalDistributionExtended(ctx: ExtendedCheckContext): Promise<PulseScanCheckInput[]> {
  ...
  checks.push({ category: "Global Distribution", checkKey: "uk_pecr_cookie_law", label: "UK PECR cookie law compliance", status: hasUkPecr ? "PASS" : "WARN", detail: hasUkPecr ? "UK PECR cookie compliance signals detected." : "No UK PECR reference — UK organisations must comply with the Privacy and Electronic Communications Regulations (PECR) for cookie consent, separate from UK GDPR." });
  ...
  checks.push({ category: "Global Distribution", checkKey: "local_legal_notice", label: "Mentions Légales (France / Belgium)", status: isFrenchSite ? (hasMentionsLegales ? "PASS" : "WARN") : "PASS", detail: isFrenchSite ? (hasMentionsLegales ? "Mentions Légales page signals detected." : "French market signals detected — French and Belgian law requires a 'Mentions Légales' page with company registration, VAT number, and hosting details.") : "Not applicable." });
