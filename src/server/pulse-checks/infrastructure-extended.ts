...
export async function runInfrastructureExtended(ctx: ExtendedCheckContext): Promise<PulseScanCheckInput[]> {
  ...
  checks.push({ category: "Infrastructure", checkKey: "ipv6_dns_record", label: "IPv6 (AAAA) DNS record present", status: hasIpv6 ? "PASS" : "WARN", detail: hasIpv6 ? "IPv6 AAAA record found — dual-stack deployment in place." : "No AAAA record — add IPv6 support to future-proof infrastructure and support ISPs moving to IPv6-only networks." });
  ...
  checks.push({ category: "Infrastructure", checkKey: "multi_region_signals", label: "Multi-region deployment signals", status: hasMultiRegion ? "PASS" : "WARN", detail: hasMultiRegion ? "Multi-region deployment signals detected." : "No multi-region signals — single-region deployments have geographic latency and no availability failover. Consider multi-region or a global CDN." });
  ...
  checks.push({ category: "Infrastructure", checkKey: "load_balancer_detected", label: "Load balancer / reverse proxy detected", status: hasLb ? "PASS" : "WARN", detail: hasLb ? "Load balancer or reverse proxy signals detected." : "No load balancer signals detected — a load balancer is required for horizontal scaling and zero-downtime deployments." });
  ...
  checks.push({ category: "Infrastructure", checkKey: "object_storage_signals", label: "Object storage (S3 / GCS) for assets", status: hasObjectStorage ? "PASS" : "WARN", detail: hasObjectStorage ? "Object storage signals detected." : "No object storage signals — use S3, GCS, or Cloudflare R2 for user uploads and static assets rather than local disk." });
