...
const LLM_ANALYSIS = {
  ...
  strengths: [
    {
      ...
      detail: "Next.js 15 App Router with Supabase and Vercel is an excellent foundation — edge-deployable, real-time capable, and well-supported. The architecture supports growth to thousands of users without a rewrite.",
    },
    {
      title: "Payments already live",
      detail: "Stripe subscription billing is integrated and processing real transactions. This is the hardest milestone for early-stage SaaS — it's done.",
    },
    {
      title: "GOV.UK API integrations",
      detail: "GOV.UK Notify integration for Right to Rent notifications is a genuine differentiator — most competitors use manual email flows. This is a strong enterprise buying signal.",
    },
    {
      title: "Core product velocity",
      detail: "3.5 commits per week with 2 active contributors shows a healthy, active development pace. The feature set is moving forward.",
    },
  ],
  criticalGaps: [
    {
  ...
  buildOpportunities: [
    {
      ...
      description: "Integrate TrueLayer or Plaid to verify tenant rental payment history directly from bank statements. Letting agencies would pay a premium for automated reference checks that replace manual PDF uploads.",
      estimatedEffort: "L",
      businessValue: "HIGH",
    ...
    },
    {
      ...
      description: "Sync property details and availability directly to portals via their API. This removes the biggest manual workflow for letting agents and is the #1 requested feature in the PropTech space.",
      estimatedEffort: "XL",
      businessValue: "HIGH",
      category: "Integrations",
    },
    {
      ...
      description: "Generate legally compliant eviction notices pre-filled from tenancy data. With Section 21 abolition pending, landlords are anxious — a compliant notice generator is a strong retention driver.",
      estimatedEffort: "M",
      businessValue: "HIGH",
    ...
    },
    {
      ...
      description: "Allow agencies to brand the tenant portal with their own logo, colours, and domain. Required by any letting agency with more than 3 staff. Unlocks a £200–£800/month tier.",
      estimatedEffort: "L",
      businessValue: "HIGH",
      category: "Platform",
    },
    {
      title: "Automated rent reminder & arrears escalation",
      description: "Send GOV.UK Notify SMS/email reminders at D-3, D-0, and D+3 of rent due date. Automate arrears escalation to letter generation. Reduces average debt recovery time by 60%.",
      estimatedEffort: "S",
      businessValue: "HIGH",
      category: "Automation",
    },
    {
      ...
      description: "Direct integration with DPS, MyDeposits, or TDS so landlords can register deposits without leaving the platform. Currently requires manual registration — a compliance risk and friction point.",
      estimatedEffort: "M",
    ...
    },
    {
      ...
      description: "Allow tenants to raise maintenance tickets, auto-notify landlords, and assign preferred contractors. The current manual process is a major source of support requests.",
      estimatedEffort: "M",
      businessValue: "MEDIUM",
    ...
    },
    {
      ...
      description: "Receive maintenance alerts, approve payments, and review tenancy documents from mobile. 68% of private landlords are over 45 and strongly prefer mobile-first experiences for notifications.",
      estimatedEffort: "XL",
      businessValue: "MEDIUM",
      category: "Platform",
    },
  ...
  techDebt: [
    {
      area: "Authentication",
      description: "jsonwebtoken 8.5.1 with CVE-2022-23529 in active use across tenant auth middleware. Upgrade to v9+ required.",
      severity: "HIGH",
    },
    {
      area: "Stripe integration",
      description: "Webhook handler at /api/stripe/webhook does not validate idempotency keys. Duplicate events are processed twice — causing ledger corruption at low frequency.",
      severity: "HIGH",
    },
    {
      area: "TypeScript",
      description: "47 @ts-ignore comments suppressing type errors, predominantly in the Supabase client layer and API route handlers.",
      severity: "MEDIUM",
    },
    {
      area: "Database queries",
      description: "Several N+1 query patterns in the tenancy dashboard — loading properties then making per-property requests for tenants and payments. Causes visible slowdown above 20 properties.",
      severity: "MEDIUM",
    },
    {
      area: "Environment variables",
      description: "7 environment variables referenced in code that are not documented in .env.example or deployment runbook.",
      severity: "LOW",
    },
  ],
  proposalHook:
  ...
  techStackAnalysis: {
    assessment:
      "The Next.js 15 / Supabase / Vercel combination is an excellent foundation for a UK SaaS — edge-ready, real-time capable, and well-aligned with UK data residency needs (Supabase EU-West region). The main gaps are observability (no Sentry), compliance tooling (no CMP, no DPA workflow), and third-party integrations (Open Banking, portal APIs) that the market expects. The authentication vulnerability is the most pressing issue and should be addressed before any marketing spend is increased.",
    detectedStack: {
    ...
    recommendations: [
      { area: "Error Monitoring", current: null, recommended: "Sentry", reason: "No error monitoring means production failures are invisible until users report them. Sentry Next.js SDK installs in 15 minutes and surfaces the 504 payroll errors immediately.", priority: "HIGH" },
      { area: "Security Headers", current: null, recommended: "next.config.js headers() + Content Security Policy", reason: "CSP prevents XSS attacks from exfiltrating tenant identity documents. HSTS ensures all connections are encrypted. Both are 30-minute implementations.", priority: "HIGH" },
      { area: "Cookie Consent", current: null, recommended: "Cookiebot or Osano", reason: "GDPR/PECR requires explicit consent before setting analytics or marketing cookies. ICO has issued enforcement notices to UK SaaS companies for this specific violation.", priority: "HIGH" },
      { area: "Caching Layer", current: null, recommended: "Redis via Upstash", reason: "Property listings and tenancy dashboards are re-queried on every page load. A Redis cache layer would cut database load by ~60% and resolve the N+1 issues without a full rewrite.", priority: "MEDIUM" },
      { area: "Background Jobs", current: null, recommended: "Trigger.dev or Inngest", reason: "Rent reminders, arrears escalation, and deposit expiry notifications need reliable background scheduling. Cron jobs on Vercel have a 60s execution limit — too short for batch rent processing.", priority: "MEDIUM" },
      { area: "Search", current: null, recommended: "Algolia or Typesense", reason: "Full-text search across properties, tenants, and documents via Postgres ILIKE is slow above 1,000 records. Algolia integration is a 1-day implementation.", priority: "LOW" },
    ],
...
const DISCOVERY_KIT = {
  ...
  wowFinding: {
    finding: "There's a high-severity JWT vulnerability (CVE-2022-23529) in your tenant authentication flow, and your Stripe webhook handler is processing duplicate events — we can see duplicate payment records in the pattern.",
    impact: "The JWT issue means an attacker could forge a tenant token and access any other tenant's identity documents and payment history. The Stripe issue has already created incorrect rent ledger entries twice in the last 30 days.",
  },
  questions: [
    {
      question: "How are you currently handling tenant identity verification — is that still a manual process?",
      context: "The GOV.UK Notify integration is impressive, but there's no Open Banking connection for automated rental reference checks. This is consistently the #1 friction point for letting agencies evaluating PropTech platforms.",
    ...
    },
    {
      ...
      context: "The platform has no white-label capability, which is a hard blocker for any letting agency with branded client communications. Understanding their scale tells us how urgently this needs to be built.",
    ...
    },
    {
      ...
      context: "There's no DPA documentation on the site. Any letting agency operating under GDPR is legally required to have a signed DPA with all data processors before onboarding. This is currently blocking enterprise deals.",
    ...
    },
    {
      ...
      context: "Vercel logs show 504 timeouts on the payroll run endpoint, but there's no Sentry or equivalent. These errors are invisible until a landlord contacts support — which means you don't know the true error rate.",
    ...
    },
    {
      ...
      context: "There's no onboarding flow — new users land on an empty dashboard with no guidance. This is the most common cause of sub-10% trial conversion in SaaS, and it's a relatively fast fix.",
    ...
    },
    {
      ...
      context: "The agency market is 10x the revenue opportunity but requires white-labelling, SSO, and a signed DPA. If agency is the direction, the compliance and platform gaps need to be on the roadmap now.",
    ...
    },
    {
      ...
      context: "There's no notice generation feature in the platform. This is a strong near-term sales angle — landlords are anxious and searching for tools that help them navigate the new regime.",
...
const AGENT_DATA = {
  codeInsights: {
    vulnerabilities: [
      { severity: "HIGH", packageName: "jsonwebtoken", description: "CVE-2022-23529 — improper validation of algorithm in jwt.verify() allows an attacker to forge tokens with a crafted public key. Affects versions < 9.0.0." },
      { severity: "MEDIUM", packageName: "undici", description: "CVE-2023-45143 — cookie header not cleared on cross-origin redirect, potentially leaking session tokens to attacker-controlled domains." },
      { severity: "MEDIUM", packageName: "undici", description: "CVE-2023-23936 — CRLF injection via hostname in undici fetch, allowing header injection in server-side API calls." },
    ],
...
const CHECKS: Array<{ category: string; checkKey: string; label: string; status: "PASS" | "WARN" | "FAIL"; detail?: string; evidence?: string }> = [
  // Infrastructure
  ...
  // Security
  { category: "Security", checkKey: "sec_csp", label: "Content Security Policy header", status: "FAIL", detail: "No CSP header. XSS attacks can exfiltrate tenant identity documents from Supabase Storage.", evidence: "content-security-policy: not present" },
  ...
  { category: "Security", checkKey: "sec_cve_medium", label: "No medium-severity CVEs in dependencies", status: "FAIL", detail: "2 medium-severity CVEs in undici (HTTP client): CVE-2023-45143, CVE-2023-23936" },
  ...
  { category: "Security", checkKey: "sec_security_txt", label: "security.txt present", status: "FAIL", detail: "No /.well-known/security.txt — no responsible disclosure contact" },
  ...
  // Performance
  { category: "Performance", checkKey: "perf_lcp", label: "LCP < 2.5s (Lighthouse)", status: "FAIL", detail: "LCP 3.8s — NEEDS IMPROVEMENT. Hero image not using next/image optimisation, render-blocking Intercom script", evidence: "Lighthouse LCP: 3800ms" },
  { category: "Performance", checkKey: "perf_cls", label: "CLS < 0.1 (Lighthouse)", status: "FAIL", detail: "CLS 0.18 — 6 next/image instances missing width/height cause layout shift on load", evidence: "Lighthouse CLS: 0.18" },
  { category: "Performance", checkKey: "perf_fcp", label: "FCP < 1.8s", status: "WARN", detail: "FCP 2.1s — slightly above threshold. Render-blocking stylesheet from Intercom widget", evidence: "Lighthouse FCP: 2100ms" },
  { category: "Performance", checkKey: "perf_tbt", label: "TBT < 200ms", status: "FAIL", detail: "TBT 420ms — long JavaScript parsing time. Uncode-split dashboard bundle", evidence: "Lighthouse TBT: 420ms" },
  ...
  { category: "Legal & Compliance", checkKey: "legal_cookie_consent", label: "Cookie consent management", status: "FAIL", detail: "No cookie consent platform — GA4 and Intercom fire before user consent. GDPR/PECR violation.", evidence: "No CMP script detected on page load." },
  ...
  // Observability
  { category: "Observability", checkKey: "obs_error_monitoring", label: "Error monitoring configured (Sentry)", status: "FAIL", detail: "No Sentry or equivalent. Production errors including 504 timeouts on /api/payroll/run are undetected.", evidence: "No @sentry/nextjs in package.json." },
  ...
  { category: "Observability", checkKey: "obs_health_endpoint", label: "Health check endpoint", status: "FAIL", detail: "No /api/health endpoint — cannot integrate with uptime monitoring" },
  ...
  { category: "SaaS Signals", checkKey: "saas_docs", label: "Product documentation / help centre", status: "WARN", detail: "Intercom knowledge base present but empty — no articles." },
  ...
  { category: "UX & Conversion", checkKey: "ux_trust_signals", label: "Trust signals visible (logos, certs, awards)", status: "WARN", detail: "No trust badges, security certifications, or client logos on homepage" },
  ...
  // PWA Signals
  { category: "PWA Signals", checkKey: "pwa_manifest", label: "Web app manifest", status: "FAIL", detail: "No manifest.json — platform cannot be installed as a PWA on mobile." },
...
export async function POST() {
  try {
    ...
    if (!workspace) return apiError("Workspace not found. Run the base seed first.", 500);

    // Idempotent — delete any existing demo scan and recreate fresh
    const existing = await prisma.pulseScan.findFirst({
      where: { workspaceId: workspace.id, inputGithubRepo: "estato-hq/estato-platform" },
      select: { id: true },
    });
    if (existing) {
    ...
    const scan = await prisma.pulseScan.create({
      data: {
        ...
        techStack: ["Next.js 15", "Supabase", "PostgreSQL", "Vercel", "Stripe", "GOV.UK Notify", "Resend", "Intercom", "GA4"] as unknown as Prisma.InputJsonValue,
