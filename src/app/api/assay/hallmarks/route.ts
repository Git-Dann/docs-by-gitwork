...
import { issueHallmark, listHallmarks } from "@/server/assay/issue";
import { canSeal } from "@/server/assay/digest";
import { hallmarkIssueSchema } from "@/server/validators";

export const dynamic = "force-dynamic";

/** List the workspace's hallmarks, newest first. */
export async function GET(request: NextRequest) {
  try {
    const user = await requireAuthedUser(request);
    const hallmarks = await listHallmarks(user.workspaceId);
    ...
    return apiOk({ hallmarks, sealingConfigured: canSeal() });
  } catch (error) {
...
/**
 * Issue a hallmark from a completed Pulse scan.
 *
...
export async function POST(request: NextRequest) {
  try {
    const user = await requireAuthedUser(request);
    assertCan(user, canIssueHallmark, "issue hallmarks");
    const { scanId, standardId } = hallmarkIssueSchema.parse(await request.json());
    const hallmark = await issueHallmark({
    ...
    return apiOk({ hallmark, sealingConfigured: canSeal() }, { status: 201 });
  } catch (error) {
    return fromError(error);
  }
}
