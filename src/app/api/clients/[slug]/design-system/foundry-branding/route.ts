...
import { setDesignSystemFoundryBranding } from "@/server/design-system";
import { designSystemFoundryBrandingSchema } from "@/server/validators";
...
export async function PUT(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const body = designSystemFoundryBrandingSchema.parse(await req.json());
    return apiOk(await setDesignSystemFoundryBranding(user, clientId, body.enabled));
  } catch (e) {
    return fromError(e);
  }
}
