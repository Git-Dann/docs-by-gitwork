...
//#region node_modules/.pnpm/react-resizable-panels@4.12.0_react-dom@19.2.7_react@19.2.7__react@19.2.7/node_modules/react-resizable-panels/dist/react-resizable-panels.js
...
function ve(e) {
	...
	return n === 0 ? t.map((o) => ({
		...
		let u = 0;
		if (s.collapsedSize !== void 0) u = O(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.collapsedSize
		}) / n * 100);
		let a;
		if (s.defaultSize !== void 0) a = O(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.defaultSize
		}) / n * 100);
		let r = 0;
		if (s.minSize !== void 0) r = O(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.minSize
		}) / n * 100);
		let l = 100;
		if (s.maxSize !== void 0) l = O(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.maxSize
		}) / n * 100);
...
function Je(e) {
	const { element: t, orientation: n, panels: o, separators: i } = e, s = be(n, Array.from(t.children).filter(qe).map((z) => ({ element: z }))).map(({ element: z }) => z), u = [];
...
function W({ layout: e, panelConstraints: t }) {
...
function tt({ groupId: e, panelId: t }) {
	const n = () => {
		...
		if (r === s()) return;
		const { defaultLayoutDeferred: c, derivedPanelConstraints: m, group: p, groupSize: S, layout: z, separatorToPanels: f } = n(), h = W({
			layout: u({
				nextSize: r,
				panels: p.panels,
				prevLayout: z,
	...
	return {
		...
		resize: (r) => {
			const { group: l } = n(), { element: c } = i(), m = ne({ group: l }), S = O(ie({
				groupSize: m,
				panelElement: c,
				styleProp: r
			}) / m * 100);
...
function nt({ groupId: e }) {
	...
	return {
		...
		setLayout(n) {
			const { defaultLayoutDeferred: o, derivedPanelConstraints: i, group: s, groupSize: u, layout: a, separatorToPanels: r } = t(), l = W({
				layout: n,
				panelConstraints: i
			});
...
function U(e, t) {
	...
	const u = s.map((m) => n.panels.indexOf(m)), r = nt({ groupId: n.id }).getLayout(), c = W({
		layout: le({
			delta: t,
			initialLayout: r,
...
function at({ document: e, event: t, hitRegions: n, initialLayoutMap: o, mountedGroups: i, pointerDownAtPoint: s, prevCursorFlags: u }) {
	let a = 0;
	n.forEach((l) => {
...
function Ue(e) {
	...
	switch (t.state) {
		case "active": te({
			...
			j(n.group, o, { isUserInteraction: !0 });
		}), e.preventDefault());
	}
}
function Be(e) {
...
function jt({ group: e, nextGroupSize: t, prevGroupSize: n, prevLayout: o }) {
	...
	if (a.forEach((m, p) => {
		c[p] = m;
	}), s > 0) for (const m of r) c[m] = O((o[m] ?? 0) / s * l);
	else {
		const m = O(l / r.length);
...
function Vt(e) {
		...
		for (const d of f) {
			...
			if (y === e.element) {
				if (t) {
					...
					const v = H(e.id);
					if (!v) return;
					const g = ve(e), w = v.defaultLayoutDeferred ? Be(g) : v.layout, L = W({
						layout: jt({
							group: e,
							nextGroupSize: b,
					...
					if (!v.defaultLayoutDeferred && B(v.layout, L) && $t(v.derivedPanelConstraints, g) && v.groupSize === b) return;
					j(e, {
	...
	const m = W({
		layout: e.mutableState.layouts[r] ?? l ?? Be(a),
		panelConstraints: a
	...
	return Je(e).forEach((f) => {
		...
		C(!i.has(f.id), `Separator ids must be unique; id "${f.id}" was used more than once`), i.add(f.id), f.element.addEventListener("keydown", Ge);
		...
		t = !1, J.set(p, Math.max(0, (J.get(p) ?? 0) - 1)), Lt(e), e.separators.forEach((d) => {
			d.element.removeEventListener("keydown", Ge);
		}), J.get(p) || (p.removeEventListener("dblclick", Te, !0), p.removeEventListener("pointerdown", Ae, !0), p.removeEventListener("pointerleave", je), p.removeEventListener("pointermove", He), p.removeEventListener("pointerout", Ve), p.removeEventListener("pointerup", Ue, !0)), s.disconnect();
	};
}
function Ut() {
...
function Wt({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: u, id: a, onLayoutChange: r, onLayoutChanged: l, orientation: c = "horizontal", resizeTargetMinimumSize: m = {
	coarse: 20,
	fine: 10
}, style: p, ...S }) {
	...
	}), h = Le(a), y = (0, import_react.useRef)(null), [b, v] = Ut(), g = (0, import_react.useRef)({
		...
		const oe = ze(h, (_) => {
			...
			R.mutableState.layouts[ut] = ce, Ee.forEach(($) => {
				if ($.collapsible) {
					...
					if (ge) {
						const ft = k($.collapsedSize, ce[$.panelId]), dt = k($.collapsedSize, ge[$.panelId]);
	...
	if (i) try {
		...
		if (t) {
			...
			if (a && Array.isArray(a.layout) && t.length === a.layout.length) {
				const r = {};
				for (let l = 0; l < t.length; l++) r[t[l]] = a.layout[l];
				return r;
			}
		} else {
			...
			if (u.length === 1) {
				...
				if (a && Array.isArray(a.layout)) {
					...
					if (r.length === a.layout.length) {
						const l = {};
						for (let c = 0; c < r.length; c++) l[r[c]] = a.layout[c];
						return l;
					}
				}
			}
...
function on({ debounceSaveMs: e = 100, onlySaveAfterUserInteractions: t, panelIds: n, storage: o = localStorage, ...i }) {
	const s = n !== void 0, u = "id" in i ? i.id : i.groupId, a = he(u, n ?? []), r = (0, import_react.useSyncExternalStore)(Xt, () => o.getItem(a), () => o.getItem(a)), l = (0, import_react.useMemo)(() => {
		if (r) {
			const d = JSON.parse(r), h = Object.values(d);
			if (Array.from(h).every((y) => typeof y == "number")) return d;
		}
		...
		n,
		o
	]), m = l ?? c, p = (0, import_react.useRef)(null), S = (0, import_react.useCallback)(() => {
		const d = p.current;
	...
	const z = (0, import_react.useCallback)((d, h) => {
		if (t && !h.isUserInteraction) return;
		S();
		let y;
	...
	return {
		...
		*/
		onLayoutChange: (0, import_react.useCallback)((d) => {
...
function Yt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: u, groupResizeBehavior: a = "preserve-relative-size", id: r, maxSize: l = "100%", minSize: c = "0%", onResize: m, panelRef: p, style: S, ...z }) {
	...
	}, P = (0, import_react.useSyncExternalStore)((R) => ze(g, R), x, x);
	let I;
...
function Qt({ children: e, className: t, disabled: n, disableDoubleClick: o, elementRef: i, id: s, style: u, ...a }) {
	const r = Le(s), l = Re({
		disabled: n,
...
//#endregion
export { Wt as Group, Yt as Panel, Qt as Separator, wt as isCoarsePointer, on as useDefaultLayout, rn as useGroupCallbackRef, sn as useGroupRef, an as usePanelCallbackRef, ln as usePanelRef };
