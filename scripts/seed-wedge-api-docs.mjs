#!/usr/bin/env node
/**
 * seed-wedge-api-docs.mjs — one-off: populate the Wedge client wiki 'API Docs'
 * page with the REAL Big Wedge Golf API (generated from the live OpenAPI schema
 * at https://apiv1.bigwedgegolf.com/api/v1/schema/ — 223 endpoints, 28 tags).
 *
 * The API Docs wiki page is a STRUCTURED viewer, not markdown — it must be sent
 * an ApiDocsContent object or it silently falls back to the api.example.com
 * placeholder (the "example data" this replaces).
 *
 * Usage:
 *   API_KEY=xxxx node scripts/seed-wedge-api-docs.mjs            # POST to prod
 *   API_KEY=xxxx node scripts/seed-wedge-api-docs.mjs --dry-run  # print only
 *   BASE_URL=http://localhost:3000 SLUG=wedge API_KEY=xxxx node scripts/seed-wedge-api-docs.mjs
 *
 * The key is the Foundry workspace API key (accepted as API_KEY or, historically,
 * NEXT_PUBLIC_API_KEY). Delete this script after the one-time seed if you like.
 */

const BASE_URL = (process.env.BASE_URL || "https://foundry.gitwork.co.uk").replace(/\/$/, "");
const SLUG = process.env.SLUG || "wedge";
...
async function main() {
  if (!DRY && !API_KEY) {
    console.error("Missing API_KEY (or NEXT_PUBLIC_API_KEY). Set it or pass --dry-run.");
    process.exit(1);
  }
  const body = { type: "API_DOCS", title: content.title, content };
  console.log(`API Docs payload: ${content.endpoints.length} endpoints across ` +
    `${new Set(content.endpoints.map((e) => e.tag)).size} tags`);
  if (DRY) {
    console.log(JSON.stringify(body, null, 2));
    console.log("\n[dry-run] not posted.");
    return;
  }
  const url = `${BASE_URL}/api/clients/${SLUG}/wiki/pages`;
  ...
  console.log(`OK (${res.status}). API Docs page seeded for "${SLUG}".`);
}
